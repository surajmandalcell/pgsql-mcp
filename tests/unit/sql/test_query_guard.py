...
def test_placeholder_scanner_handles_escapes_and_literal_percent() -> None:
...
@pytest.mark.asyncio
async def test_safe_query_executor_reports_timeout() -> None:
    base_driver = AsyncMock(spec=SqlDriver)
    base_driver.execute_bounded_query.side_effect = TimeoutError
    executor = SafeQueryExecutor(base_driver, timeout_seconds=3)

    with pytest.raises(ValueError, match="timed out after 3 seconds"):
