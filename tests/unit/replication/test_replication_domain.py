...
def test_standby_and_receiver_payloads_preserve_operational_fields() -> None:
    item = standby()
    wal_receiver = receiver()

    assert item.to_payload()["replay_lag_seconds"] == 0.3
    assert item.to_payload()["client_address"] == "10.0.0.2"
    assert wal_receiver.to_payload()["latest_end_time"] == NOW.isoformat()
    assert wal_receiver.to_payload()["sender_host"] == "primary.internal"


def test_slot_subscription_publication_and_finding_payloads() -> None:
    finding = ReplicationFinding(
        "slot_warning",
        FindingSeverity.WARNING,
        "Slot retains WAL.",
        {"retained_wal_bytes": 42},
    ...
    assert finding.to_payload() == {
        "code": "slot_warning",
        "severity": "warning",
        "summary": "Slot retains WAL.",
        "evidence": {"retained_wal_bytes": 42},
        "recommended_action": "Inspect the consumer.",
    }
...
def test_healthy_primary_is_ready() -> None:
    ...
    assert result.to_payload()["topology_summary"] == {
        "role": "primary",
        "standby_count": 1,
        "slot_count": 1,
...
def test_primary_finds_sync_loss_state_lag_and_time_lag() -> None:
    lagging = standby(state="catchup", replay_lag_bytes=2_000, replay_lag_seconds=20)
    thresholds = ReplicationThresholds(
        warning_lag_bytes=100,
        critical_lag_bytes=1_000,
        warning_lag_seconds=10,
        critical_lag_seconds=30,
    )
    result = assess_failover_readiness(
        topology(synchronous_standby_names_configured=True, standbys=(lagging,)),
        thresholds,
    )
...
def test_standby_receiver_replay_and_lag_failures_are_critical() -> None:
    thresholds = ReplicationThresholds(warning_lag_bytes=10, critical_lag_bytes=100)
    result = assess_failover_readiness(
        topology(
            role=NodeRole.STANDBY,
            wal_receiver=receiver("stopped"),
            replay_paused=True,
            replay_lag_bytes=100,
        ),
        thresholds,
    )
...
def test_slots_subscriptions_and_visibility_produce_bounded_findings() -> None:
    thresholds = ReplicationThresholds(
        warning_inactive_slot_bytes=100,
        critical_inactive_slot_bytes=1_000,
    )
    result = assess_failover_readiness(
        topology(
            slots=(
                slot(slot_name="inactive", active=False, retained_wal_bytes=100),
                slot(slot_name="lost", wal_status="lost"),
                slot(slot_name="conflict", conflicting=True),
            ),
    ...
    assert result.status == "critical"
    assert result.to_payload()["topology_summary"]["visibility_complete"] is False
